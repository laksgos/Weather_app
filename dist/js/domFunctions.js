...
export const updateDisplay = (weatherJson, locationObj) => {
  ...
  const ccArray = creatCurrentConditionsDiv(weatherJson, locationObj.getUnit())
...
const creatCurrentConditionsDiv = (weatherObj, unit) => {
  const tempUnit = unit === 'imperial' ? 'F' : 'C'
  ...
  const feels = createElem('div', 'feels', `Feels Like ${Math.round(Number(weatherObj.current.feels_like))} °${tempUnit}`)
  const maxTemp = createElem('div', 'maxtemp', `High ${Math.round(Number(weatherObj.daily[0].temp.max))} °${tempUnit}`)
  const minTemp = createElem('div', 'mintemp', `Low ${Math.round(Number(weatherObj.daily[0].temp.min))} °${tempUnit}`)
...
const createDailyForecastDivs = (dayWeather) => {
  ...
  const dayHigh = createElem('p', 'dayHigh', `${Math.round(Number(dayWeather.temp.max))} °${tempUnit}`)
  const dayLow = createElem('p', 'dayLow', `${Math.round(Number(dayWeather.temp.min))} °${tempUnit}`)
